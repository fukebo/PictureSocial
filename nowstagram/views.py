...
from qiniuyun import qiniu_upload_file
...
def login():
	...
	if username==''or password=='':
		return redirect_msg('/regloginpage/', u'用户名或密码不能为空', 'reglogin')

	user = User.query.filter_by(username=username).first()
	if user==None:
		return redirect_msg('/regloginpage/',u'用户名不存在','reglogin')

	m=hashlib.md5()
	m.update(password + user.salt)
	if m.hexdigest() != user.password:
		return redirect_msg('/regloginpage/', u'密码错误', 'reglogin')
...
def reg():
	...
	if username == '' or password == '':
		return redirect_msg('/regloginpage/', u'用户名或密码不能为空', 'reglogin')
	if user != None:
		return redirect_msg('/regloginpage/',u'用户名已经存在','reglogin')

	salt = ''.join(random.sample('0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRETUVWXYZ', 10))
	m = hashlib.md5()
